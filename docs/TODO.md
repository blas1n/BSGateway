...
| E5b | Extend MCP injection to opencode (`opencode serve` MCP support — investigate session schema). codex MCP support pending CLI feature. | `worker/executors.py::OpenCodeExecutor.execute` | BSNexus / external user picks `opencode` executor and needs MCP | Open |
| E6 | Workspace-dir wire from `metadata.workspace_dir` to all executors' `cwd`. Defaults to `"."` when omitted. Validation (absolute-only, no `..`) deferred to caller. | `worker/main.py::_handle_task`, `bsgateway/executor/dispatcher.py::dispatch_task` | Done together with E5a — BSNexus needs the worker to operate inside its repo worktree | **Done (2026-05-03)** |
